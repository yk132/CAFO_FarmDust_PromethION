...
JOBID_2=$(sbatch --parsable --dependency=afterok:${JOBID_1} --job-name=dorado --output="$LOG_DIR/%x.%j.out" --error="$LOG_DIR/%x.%j.err" Job2_dorado_fastq.sh)

# Step 3: Demux: add dependency later!!
JOBID_3=$(sbatch --parsable --job-name=demux --dependency=afterok:${JOBID_1}:${JOBID_2} --output="$LOG_DIR/%x.%j.out" --error="$LOG_DIR/%x.%j.err" Job3_demux.sh)

# Merge FASTQ files and check for quality with fastQC
JOBID_4=$(sbatch --parsable --job-name=fastqc --dependency=afterok:${JOBID_1}:${JOBID_2}:${JOBID_3} --output="$LOG_DIR/%x.%j.out" --error="$LOG_DIR/%x.%j.err" Job4_merge_fastqc.sh)
